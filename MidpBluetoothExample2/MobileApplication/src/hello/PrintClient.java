...
public class PrintClient implements DiscoveryListener {
     ...
     * initialized
     */
    public PrintClient(Form s) throws BluetoothStateException {
        mydebug=s;
...
System.out.println("Record is null");
    ...
    public ServiceRecord findPrinter() {
        ...
        debug("findPrinter ndevices: " + deviceList.size());
        ...
        if (deviceList.size() > 0) {
            ...
            if (searchServices(devList)) {
                debug("findPrinter found service");
                return record;
            }
        } else {
...
if (record == null) {
	...
	for(int i=0; i < servRecord.length; i++) {
		...
		if(de != null && de.getDataType() == DataElement.STRING) {
			...
			debug("found service:"+srvName+"\n");
		} else {
			debug("found service:"+sr.getConnectionURL(0,false)+"\n");
		}
	}
}
