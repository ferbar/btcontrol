...
void BTServer::pushUpdate(int so)
{
	std::string clientAddr = BTServer::getRemoteAddr(so);
	for(int i=0; i < 10; i++) {
		printf("."); fflush(stdout);
		sleep(1);
	}
	BTServer::BTPush(clientAddr);
}

void BTServer::BTPush(std::string addr)
{
...
void BTServer::BTScan(FBTCtlMessage &reply)
{
