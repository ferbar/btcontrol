/*ESP32_MAS
  ...
  "ESP32_MAS.openFile(uint8_t channel, File &f)"
  Opens the next file, can be overridden. Called from the Audio_Player thread. 
...
class ESP32_MAS {
  public:
    ESP32_MAS();
    virtual ~ESP32_MAS() { this->stopDAC(); } // fixme: wait until thread terminated
